...
source "googlecompute" "XS" {
  ...
  image_description       = "Sourcegraph Compute Image XS ${local.image_version} - ${formatdate("YYYYMMDD", timestamp())}"
  source_image            = "ubuntu-2204-jammy-v20221101a"
  ssh_username            = "sourcegraph"
  machine_type            = var.instance_sizes.xs.instance_type_gcp
  region                  = var.location.gcp.region
...
source "googlecompute" "S" {
  ...
  image_description       = "Sourcegraph Compute Image S ${local.image_version} - ${formatdate("YYYYMMDD", timestamp())}"
  source_image            = "ubuntu-2204-jammy-v20221101a"
  ssh_username            = "sourcegraph"
  machine_type            = var.instance_sizes.xs.instance_type_gcp
  region                  = var.location.gcp.region
...
source "googlecompute" "M" {
  ...
  image_description       = "Sourcegraph Compute Image M ${local.image_version} - ${formatdate("YYYYMMDD", timestamp())}"
  source_image            = "ubuntu-2204-jammy-v20221101a"
  ssh_username            = "sourcegraph"
  machine_type            = var.instance_sizes.xs.instance_type_gcp
  region                  = var.location.gcp.region
...
source "googlecompute" "L" {
  ...
  image_description       = "Sourcegraph Compute Image L ${local.image_version} - ${formatdate("YYYYMMDD", timestamp())}"
  source_image            = "ubuntu-2204-jammy-v20221101a"
  ssh_username            = "sourcegraph"
  machine_type            = var.instance_sizes.xs.instance_type_gcp
  region                  = var.location.gcp.region
...
source "googlecompute" "XL" {
  ...
  image_description       = "Sourcegraph Compute Image XL ${local.image_version} - ${formatdate("YYYYMMDD", timestamp())}"
  source_image            = "ubuntu-2204-jammy-v20221101a"
  ssh_username            = "sourcegraph"
  machine_type            = var.instance_sizes.xs.instance_type_gcp
  region                  = var.location.gcp.region
...
build {
  ...
  post-processors {
    ...
    post-processor "googlecompute-export" {
      ...
      disk_size    = 100
      disk_type    = "pd-ssd"
      paths        = ["gs://sourcegraph-images/latest/sourcegraph-${lower(source.name)}-${local.image_version}.tar.gz"]
    }
  }
}
