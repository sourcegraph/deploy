...
func (d *Disk) GetFileSystemType(ctx context.Context) (string, error) {
	...
	if err != nil {
		return "", errors.Newf("failed to get device file system type: %v", err)
	}
...
func (d *Disk) Mount(ctx context.Context) error {
	...
	if err != nil {
		return errors.Newf("failed to mount device %s: %v", d.Device, err)
	}
...
func (d *Disk) createXFSFilesystem(ctx context.Context) error {
	...
	if err != nil {
		return errors.Newf("failed to create XFS filesystem: %v", err)
	}
...
func (d *Disk) createEXT4Filesystem(ctx context.Context) error {
	...
	if err != nil {
		return errors.Newf("failed to create EXT4 filesystem: %v", err)
	}
...
func updateFStab(mountOpts string) error {
	...
	if err != nil {
		return errors.Newf("failed to update /etc/fstab: %v", err)
	}
	defer func() {
		_ = f.Close()
	}()

	_, err = fmt.Fprint(f, mountOpts)
	if err != nil {
		return errors.Newf("failed to update /etc/fstab: %v", err)
	}
